...
//  Local functions
//-----------------------------------------------------------------------------
static void execRouting(double maxTimeStep);
...
// --- to be safe, reset the state of the floating point unit                  //(5.1.013)
...
#endif
    {
		...
		/* project_open(f1, f2, f3)包含以下三个函数
		{
		...
		*/
        ...
        report_writeLogo();//向report文件中输入log字符串 
		...
		*/
		...
		writecon(FMT06);//控制台输出  "\n o  Retrieving project data"

        // --- retrieve project data from input file
		project_readInput(); 
		/* project_readInput(); 主要包含以下内容

		{
			createHashTables();
			input_countObjects();
			createObjects();input_readData();
		...
		*/
        ...
        report_writeTitle();//向Report文件中输入Title模块的标题(至多三行)
        project_validate();
...
void execRouting(double TimeStep)
//
//  Input:   none
//  Output:  none
...
//
{
...
#endif
    {
		...
		//==================coupling version time control==================
		//if (!DoRouting) {
		//
		//}//do nothing                    MZ
  //      else maxTimeStep = routing_getRoutingStep(RouteModel, *maxTimeStep);

		routingStep = TimeStep;
		//=================================================================
		...
		//==================swmm version time control====================== ignore this part because flood modelControl make sure this situation won't happen 2020-6
		//																	dont ignore this part 2020-7-21 避免下面的DoRunoff在模拟结束前最后一次运行此函数时进入死循环
        ...
        if ( DoRunoff ) while ( NewRunoffTime < nextRoutingTime )
        {
            runoff_execute();
            if ( ErrorCode ) return;
			/*if (nextRoutingTime > 84600*1000.0) printf("%s%f\n", "NewRunoffTime", NewRunoffTime);*/ // 2020 -7-21调试bug
        }
		...
		/*climate_setState(getDateTime(NewRoutingTime)); */                                      //MZ
