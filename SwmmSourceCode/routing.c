...
//
{
	...
	/*======================SWMM initialize lateral inflow======================
    ...
    if ( BetweenEvents == FALSE )
    {
        ...
        addWetWeatherInflows(OldRoutingTime);//if ( Nobjects[SUBCATCH] == 0 ) return;
        addGroundwaterInflows(OldRoutingTime);//if ( Nobjects[SUBCATCH] == 0 ) return;
        addLidDrainInflows(OldRoutingTime);//if ( Nobjects[SUBCATCH] == 0 ) return;
        addRdiiInflows(currentDate);// if (NumRdiiNodes == 0) return 0;
        addIfaceInflows(currentDate); //if (Finflows.mode != USE_FILE) return;
