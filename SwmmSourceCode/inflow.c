...
//
{
    ...
    param = project_findObject(POLLUT, tok[1]);//第二个字段是pollut对象的id或"INFLOW"字符串
    if ( param < 0 )//如果不存在这样的pollut对象id，则认为是flow_inflow,param=-1
    {
    ...
    if ( strlen(tok[2]) > 0 )//第三个字段是时间序列的id或""空串
    {
	...
	if (param == -1)//将type赋值为5，代表FLOW_INFLOW
	{
		type = FLOW_INFLOW;
	}
