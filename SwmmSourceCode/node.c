...
//
{
    ...
    switch (type)
    {
      ...
      case OUTFALL:
        ...
        Outfall[k].tideCurve   = (int)x[3];//Curve索引
        Outfall[k].stageSeries = (int)x[4];//Time series索引
        Outfall[k].hasFlapGate = (char)x[5];//1有 0没有
        Outfall[k].routeTo     = (int)x[6];
        Outfall[k].wRouted     = NULL;
        if ( Outfall[k].routeTo >= 0 )//如果存在一个排放区域
        {
            Outfall[k].wRouted =
                (double *) calloc(Nobjects[POLLUT], sizeof(double));//创建污染物对象？？
        }
        break;
      ...
      case STORAGE:
		...
		// 版本更替问题！
        // Surcharge depth replaces ponded area     重置surcharge depth       //(5.1.013)
...
//     nodeID  elev  maxDepth  initDepth  surDepth  aPond 
{
    ...
    if ( ntoks < 2 ) return error_setInpError(ERR_ITEMS, "");//至少得有2个值，否则报错
    ...
    for ( i = 1; i <= 5; i++ )
    {
		/* 将TOK[]中的值赋给x[]，若TOK[]中不存在该值，则设为0.0 */
        x[i-1] = 0.0;
        if ( i < ntoks )//！！关键
        {
...
//
{
    ...
    // --- get ID name
    if ( ntoks < 6 ) return error_setInpError(ERR_ITEMS, "");//最少有6个字段
    id = project_findID(NODE, tok[0]);
    if ( id == NULL ) return error_setInpError(ERR_NAME, tok[0]);
	...
	// 注意这里 好像有个版本更替的问题？ ponded area => surcharge depth  
...
//
{
    ...
    i = findmatch(tok[2], OutfallTypeWords); //outfall type   :{ w_FREE, w_NORMAL, w_FIXED, w_TIDAL,w_TIMESERIES, NULL}
    if ( i < 0 ) return error_setInpError(ERR_KEYWORD, tok[2]);
    x[1] = i;                                              // outfall type
    x[2] = 0.0;                                            // fixed stage
    x[3] = -1.;                                            // tidal curve 潮汐曲线
    x[4] = -1.;                                            // tide series 潮流系列
    x[5] = 0.;                                             // flap gate  舌瓣闸门
    x[6] = -1.;                                            // route to subcatch//(5.1.008)


	/*    Tok[0]     Tok[1]           Tok[2]                   Tok[3]                      Tok[4]            Tok[5]        */
	/*      ↓          ↓                ↓                        ↓                           ↓                 ↓          */
	/*     Id     invert_Elev.    outfall_Type    fixed_Stage/tidal_Curve/time_Series    flap_Gate   route_to_subcatch   */

    n = 4;// 四个必要数据: 1、Id  2、invert_Elev.  3、outfall_Type   4、flap_Gate
    if ( i >= FIXED_OUTFALL )
    {
        if ( ntoks < 4 ) return error_setInpError(ERR_ITEMS, "");//只有前三个数据，则缺少数据
        n = 5;//五条数据 添加一个固定水位值/曲线索引/时间序列索引
        switch ( i )
        {
        ...
        case TIDAL_OUTFALL:                                // tidal curve
          m = project_findObject(CURVE, tok[3]);  //m为名为tok[3]的CURVE的索引            
          if ( m < 0 ) return error_setInpError(ERR_NAME, tok[3]);
          x[3] = m;//int转double
          break;

        case TIMESERIES_OUTFALL:                           // stage time series
          m = project_findObject(TSERIES, tok[3]); //  m为名为tok[3]的Time Series的索引                  
          if ( m < 0 ) return error_setInpError(ERR_NAME, tok[3]);
          x[4] = m;//int转double
          Tseries[m].refersTo = TIMESERIES_OUTFALL;//将该Time Series的类型设为出水口时间序列
        }
    }
    if ( ntoks == n ) //说明没有 route_to_subcatch的数据
    {
    ...
    if ( ntoks == n+1)//有 route_to_subcatch的数据
    {
        m = project_findObject(SUBCATCH, tok[n]);//找到一个子汇水区的索引
