...
//
{
	...
	// p[0]=Geo1  p[1]=Geo2  p[2]=Geo3  p[3]=Geo4
    ...
    case CIRCULAR:
        xsect->yFull = p[0]/ucf;//管道截面 最大深度 单位换算
        xsect->wMax  = xsect->yFull;//管道截面 最大宽度
        xsect->aFull = PI / 4.0 * xsect->yFull * xsect->yFull;// 0.25piD²
        xsect->rFull = 0.2500 * xsect->yFull;//管道截面 水力半径 0.25D
        xsect->sFull = xsect->aFull * pow(xsect->rFull, 2./3.);//截面形状系数 A*R^2/3 断面面积×水力半径的三分之二次方
        xsect->sMax  = 1.08 * xsect->sFull;//最大流量时的截面形状系数 1.08倍的界面形状系数
        xsect->ywMax = 0.5 * xsect->yFull;//最大宽度位置的水平线高度
        break;
