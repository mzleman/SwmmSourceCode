...
//
{
    switch ( type )
    {
      case CONDUIT: return conduit_readParams(j, k, tok, ntoks);//type==0
      case PUMP:    return pump_readParams(j, k, tok, ntoks);//type==1
      case ORIFICE: return orifice_readParams(j, k, tok, ntoks);//type==2
      case WEIR:    return weir_readParams(j, k, tok, ntoks);//type==3
      case OUTLET:  return outlet_readParams(j, k, tok, ntoks);//type==4
      default: return 0;
    }
}
...
//
{
    ...
    // --- get index of link
    if ( ntoks < 6 ) return error_setInpError(ERR_ITEMS, "");//至少有六个字段
    ...
    {
        // --- parse max. depth & shape curve for a custom shape //定制形状 和不规则图形的区别是会有一个max. depth字段
        if ( k == CUSTOM )
        {
        ...
        else for (i = 2; i <= 5; i++) //其他允许的规则图形，读取四个形状参数Geo1 Geo2 Geo3 Geo4
        {
        ...
        if ( Link[j].type != CONDUIT && k == RECT_OPEN )//矩形的非导水管 只考虑Geo1 Geo2 忽略 Geo3 Geo4
        {
            x[2] = 0.0;
            x[3] = 0.0;
        }
        ...
        if ( Link[j].type == CONDUIT && ntoks >= 7 ) //如果存在第七个字段 => Barrels =>Conduit[k].barrels = (char) Barrels
        {
            i = atoi(tok[6]);
        ...
        if ( Link[j].type == CONDUIT && ntoks >= 8 ) //如果存在第八个字段 => culvert code => Link[j].xsect.culverCode = (int)culvert code
        {
            i = atoi(tok[7]);
...
//
{
    ...
    if ( ntoks < 4 ) return error_setInpError(ERR_ITEMS, "");//至少得有4个字段
    j = project_findObject(LINK, tok[0]);//第一个字段 => 管线的id => 查找该LInk的索引
    if ( j < 0 ) return error_setInpError(ERR_NAME, tok[0]);
    for (i=1; i<=3; i++)
    {
        if ( ! getDouble(tok[i], &x[i-1]) || x[i-1] < 0.0 ) //读取第二~第四个字段，即三个损失系数 赋值给x[0] x[1] x[2]
        return error_setInpError(ERR_NUMBER, tok[i]);
    }
    k = 0;
    if ( ntoks >= 5 ) //判断是否有第五个字段 是否存在舌瓣阀门
    {
        k = findmatch(tok[4], NoYesWords);// k=0/1
        if ( k < 0 ) return error_setInpError(ERR_KEYWORD, tok[4]);
    }
    if ( ntoks >= 6 ) //判断是否有第六个字段 渗漏率
    {
    ...
    Link[j].seepRate     = seepRate / UCF(RAINFALL);//渗漏率
    return 0;
}
...
//
{
    ...
    if ( ntoks < 7 ) return error_setInpError(ERR_ITEMS, "");//最少有7个字段

    id = project_findID(LINK, tok[0]);                // link ID
    if ( id == NULL ) return error_setInpError(ERR_NAME, tok[0]);

    n1 = project_findObject(NODE, tok[1]);            // upstrm. node 第二个字段 => 入口节点的id => 查找入口节点的索引n1
    if ( n1 < 0 ) return error_setInpError(ERR_NAME, tok[1]);

    n2 = project_findObject(NODE, tok[2]);            // dwnstrm. node 第三个字段 => 除口节点的id => 查找入口节点的索引n1
    if ( n2 < 0 ) return error_setInpError(ERR_NAME, tok[2]);

    // --- parse length & Mannings N 
    if ( !getDouble(tok[3], &x[0]) ) //读取第四个字段 => 管线长度 =>赋值给x[0]
        return error_setInpError(ERR_NUMBER, tok[3]);

    if ( !getDouble(tok[4], &x[1]) ) //读取第五个字段 = > 曼宁系数 = > 赋值给x[1]
        return error_setInpError(ERR_NUMBER, tok[4]);


    // --- parse offsets  
	//？？？后面注意一下LinkOffsets == ELEV_OFFSET时，管线的偏移是怎么处理的
	//*星号代表缺失该值，该值为必须的字段，但可以用星号代表缺省值
    if ( LinkOffsets == ELEV_OFFSET && *tok[5] == '*' ) x[2] = MISSING; 
    else if ( !getDouble(tok[5], &x[2]) )//第六个字段 => 高度偏移量 => 赋值给x[2]
        return error_setInpError(ERR_NUMBER, tok[5]);

    if ( LinkOffsets == ELEV_OFFSET && *tok[6] == '*' ) x[3] = MISSING;
    else if ( !getDouble(tok[6], &x[3]) )//第七个字段 => 高度偏移量 => 赋值给x[3]
        return error_setInpError(ERR_NUMBER, tok[6]);

   // --- parse optional parameters
    x[4] = 0.0;                                       // init. flow
    if ( ntoks >= 8 )//如果存在第八个字段 => 初始流量 => 赋值给x[4]，如果不存在:x[4]=0.0
    {				
        if ( !getDouble(tok[7], &x[4]) )
        return error_setInpError(ERR_NUMBER, tok[7]);
    }
    x[5] = 0.0;
    if ( ntoks >= 9 )//如果存在第九个字段 => 最大流量 => 赋值给x[5]，如果不存在:x[5]=0.0
    {
