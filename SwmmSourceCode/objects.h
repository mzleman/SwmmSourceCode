...
typedef struct
{
   ...
   double        fracImperv;      // fraction impervious 不透水率？？
...
typedef struct
{
   ...
   double        X;                                          //MZ
   double	     Y;                                          //MZ 
   double        weirB;                                      //MZ  堰流公式中的堰宽B,单位m^2
   double        cqA;                                     //MZ  孔口出流公式中的面积A，单位m^2
}  TNode;
...
typedef struct
{
   ...
   double      aCoeff;            // coeff. of area v. height curve 多项式系数
   double      aExpon;            // exponent of area v. height curve 指数系数
   int         aCurve;            // index of tabulated area v. height curve 索引
   TExfil*     exfil;             // ptr. to exfiltration object 蓄水设施渗出模型对象
   //-----------------------------
   double      hrt;               // hydraulic residence time (sec) 水力停留时间
   double      evapLoss;          // evaporation loss (ft3) 蒸发率
   double      exfilLoss;         // exfiltration loss (ft3) 渗出率
}  TStorage;
...
typedef struct
{
   ...
   double        sFull;           // section factor when full (ft^4/3) 截面形状系数
   double        sMax;            // section factor at max. flow (ft^4/3) 最大流量时的截面形状系数
...
typedef struct
{
   ...
   double        qLimit;          // constraint on max. flow (cfs) 极限流量
   double        cLossInlet;      // inlet loss coeff.  入口损失系数
   double        cLossOutlet;     // outlet loss coeff. 出口损失系数
   double        cLossAvg;        // avg. loss coeff.   沿程损失系数
   double        seepRate;        // seepage rate (ft/sec) 渗漏率
   int           hasFlapGate;     // true if flap gate present 舌瓣闸门
   //-----------------------------
   ...
   double        surfArea1;       // upstream surface area (ft2) 上游水表面积
   double        surfArea2;       // downstream surface area (ft2) 下游水表面积
   double        qFull;           // flow when full (cfs) 管线满时的流量
   ...
   double        froude;          // Froude number 佛劳德数
   double*       oldQual;         // previous quality state
   double*       newQual;         // current quality state
   double*       totalLoad;       // total quality mass loading
   int           flowClass;       // flow classification 流态 急流/缓流
   double        dqdh;            // change in flow w.r.t. head (ft2/sec)
   signed char   direction;       // flow direction flag 流向
